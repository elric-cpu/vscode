...
import { createEstimateVersion, insertEstimateData } from "../services/estimateService.js";
...
export const handleRecompute = async (req, res) => {
  try {
    const { estimate_version_id: estimateVersionId, answers = [] } = req.body || {};
    ...
    for (const item of updatedLineItems) {
      ...
      const materialCost = Number((price.unit_price * item.quantity).toFixed(2));
      const laborCost = labor.labor_cost;
      const allowances = item.allowances || { equipment: 0, disposal: 0 };
      const baseTotal = materialCost + laborCost + (allowances.equipment || 0) + (allowances.disposal || 0);
      const lineTotal = Number((baseTotal * (1 + (item.markup_percent || 0))).toFixed(2));
