...
import { upsertDocumentParse, createExcerpts } from "../services/documentService.js";
import { getOrCreateEstimate, createEstimateVersion, insertEstimateData } from "../services/estimateService.js";
import { writeAudit } from "../services/auditService.js";
import { logError } from "../utils/logger.js";

export const handleExtract = async (req, res) => {
  try {
    const { document_id: documentId, organization_id: organizationId, project_id: projectId, requested_by: requestedBy } = req.body || {};
    ...
    const pageIndex = new Map(parsed.pages.map((page) => [page.page_number, page]));
    ...
    return res.json({ ok: true, estimate_id: estimate.id, version_id: version.id });
  } catch (error) {
