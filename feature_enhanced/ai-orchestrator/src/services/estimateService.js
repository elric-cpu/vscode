...
export const getOrCreateEstimate = async ({ organizationId, projectId, requestedBy }) => {
...
export const createEstimateVersion = async ({ estimateId, organizationId, totals, createdBy }) => {
...
export const insertEstimateData = async ({ versionId, organizationId, tasks, lineItems, excerptMap }) => {
  if (tasks.length) {
    await supabaseAdmin.from("estimate_tasks").insert(
      tasks.map((task) => ({
        ...
        source_excerpt_id:
          ...
          (task.source_ref
            ? excerptMap.get(`${task.source_ref.page}|${task.source_ref.excerpt}`)
            : null),
      })),
    );
  }
    ...
    await supabaseAdmin.from("estimate_line_items").insert(
      lineItems.map((item) => ({
        ...
        source_excerpt_id:
          ...
          (item.source_ref
            ? excerptMap.get(`${item.source_ref.page}|${item.source_ref.excerpt}`)
            : null),
      })),
    );
  }
