...
const buildExcerpt = (page, pageText, excerpt) => {
  ...
  const end = index >= 0 ? start + excerpt.length : page.start_offset + excerpt.length;
  return {
    page_number: page.page_number,
    start_offset: start,
    end_offset: end,
...
export const upsertDocumentParse = async (documentId, organizationId, parsed) => {
...
export const createExcerpts = async ({ documentId, organizationId, pageIndex, sourceRefs }) => {
