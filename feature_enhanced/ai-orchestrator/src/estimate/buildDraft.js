...
export const buildDraftEstimate = async ({ organizationId, project, taskCandidates }) => {
  const tasks = [];
  const lineItems = [];
  let totalMaterial = 0;
  let totalLabor = 0;
  ...
  for (const candidate of taskCandidates) {
    ...
    const itemKey = normalizeKey(`${candidate.trade}_${candidate.action}_${candidate.object}`) || taskKey;
    ...
    const baseTotal = materialCost + laborCost + allowances.equipment + allowances.disposal;
