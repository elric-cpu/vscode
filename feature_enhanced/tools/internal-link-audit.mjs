...
const main = async () => {
  ...
  const objectIdentifierRoutes = {
    ...
    MAINTENANCE_LINKS: Object.fromEntries(
      Object.entries(MAINTENANCE_LINKS).map(([key, value]) => [
        key,
        value.to,
      ]),
    ),
  ...
  for (const [file, links] of pageLinks.entries()) {
    ...
    if (links.size < MIN_OUT) {
      lowOut.push({ file, count: links.size, routes: [...(fileToRoutes.get(file) || [])] });
    }
  }
  ...
  console.log(`Indexable routes with < ${MIN_IN} internal links in: ${lowIn.length}`);
