...
vi.mock("@/contexts/SupabaseAuthContext", () => ({ useAuth: () => mockUseAuth() }));
...
describe("ProtectedRoute", () => {
    ...
    render(
      <MemoryRouter initialEntries={["/protected"]}>
        <Routes>
          <Route
            path="/protected"
            element={<ProtectedRoute role="client"><div>Secret</div></ProtectedRoute>}
          />
  ...
  it("redirects to role portal when role mismatch", () => {
    mockUseAuth.mockReturnValue({ user: { user_metadata: { role: "subcontractor" } }, loading: false });
    render(
      <MemoryRouter initialEntries={["/protected"]}>
        <Routes>
          <Route
