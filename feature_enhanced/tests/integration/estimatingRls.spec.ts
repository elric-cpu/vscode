...
const setupClient = () => createClient(env.url, env.anon, { auth: { persistSession: false } });
...
(hasEnv ? describe : describe.skip)("estimating RLS", () => {
  it("isolates projects by organization", async () => {
    ...
    const { data: projectsA } = await clientA.from("projects").select("id, organization_id");
    const { data: projectsB } = await clientB.from("projects").select("id, organization_id");

    expect(projectsA?.some((project) => project.id === "66666666-6666-6666-6666-666666666666")).toBe(true);
    expect(projectsA?.some((project) => project.id === "77777777-7777-7777-7777-777777777777")).toBe(false);

    expect(projectsB?.some((project) => project.id === "77777777-7777-7777-7777-777777777777")).toBe(true);
    expect(projectsB?.some((project) => project.id === "66666666-6666-6666-6666-666666666666")).toBe(false);
  });
});
