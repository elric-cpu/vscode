...
const InstantRepairCostCalculator = () => {
  ...
  const results = useMemo(() => {
    const base = REPAIR_COSTS[systemType]?.[severity] || REPAIR_COSTS.ac.standard;
    const age = toNumber(systemAge, 0);
    const ageFactor = age >= 15 ? 1.1 : 1;
    const afterHoursFactor = afterHours ? 1.25 : 1;
    const min = base.min * localization.costFactor * ageFactor * afterHoursFactor;
    const max = base.max * localization.costFactor * ageFactor * afterHoursFactor;
        ...
        <div className="bg-cream border border-gray-200 rounded-xl p-6 space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              If you smell gas, see sparks, or have active leaks, call immediately.
            </div>
          </div>
        </div>
      </ResultsLock>
