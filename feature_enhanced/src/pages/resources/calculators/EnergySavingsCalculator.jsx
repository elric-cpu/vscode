...
const EnergySavingsCalculator = () => {
  ...
  }, [monthlyBill, savingsPercent, upgradeType, buildingType, localization.costFactor]);
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      toast({
        ...
        description: error?.message || "Please try again or call (541) 321-5115.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
    ...
    >
      <div className="bg-cream border border-gray-200 rounded-xl p-6">
        ...
        <p className="text-xs text-gray-500 mt-3 mb-0">
          ZIP-adjusted factor: {localization.costFactor.toFixed(3)}x · {localization.regionLabel}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          ...
          <p className="text-xs text-gray-500 m-0">ZIP-adjusted installed cost</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          <p className="text-xs uppercase tracking-wide text-gray-500 m-0">
            Monthly savings
          ...
          <p className="text-xs text-gray-500 m-0">Annual: {formatCurrency(results.annualSavings)}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          <p className="text-xs uppercase tracking-wide text-gray-500 m-0">
            ROI estimate
          ...
          <p className="text-xs text-gray-500 m-0">
            Payback: {results.paybackYears ? `${results.paybackYears.toFixed(1)} yrs` : "N/A"}
          </p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex justify-between text-xs text-gray-500 mb-1">
              ...
              <span>{formatCurrency(toNumber(monthlyBill, 0) - results.monthlySavings)}</span>
            </div>
            <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-gray-400"
                style={{ width: `${Math.max(10, 100 - toNumber(savingsPercent, 0))}%` }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <p className="text-sm text-restoration-gray mb-3">
          A 6,500 sq ft office upgraded to LED lighting and smart controls. Energy bills
          dropped by 21% within the first year.
        </p>
        <p className="text-xs text-gray-500 m-0">
