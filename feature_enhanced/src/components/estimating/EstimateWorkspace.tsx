...
import { calculateLaborHours, calculateLineItemTotal, calculateMaterialCost } from "@/lib/estimating/estimateMath";
import { shouldShowQuestion } from "@/lib/estimating/questionLogic";
import type { EstimateLineItem, EstimateQuestion, EstimateTask } from "@/lib/estimating/types";
...
const EstimateWorkspace = () => {
  ...
  const [answersById, setAnswersById] = useState<Record<string, string | number | null>>({});
  ...
  const answersByKey = useMemo(() => {
    return questions.reduce<Record<string, string | number | null>>((acc, question) => {
  ...
  const handleAnswerChange = async (question: EstimateQuestion, value: string | number) => {
    if (!user) return;
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12 space-y-6">
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-semibold text-gray-900">AI Estimating Workspace</h2>
        <p className="text-sm text-gray-600 mt-1">
          Upload an inspection report to generate tasks, answer targeted questions, and review a
          citation-backed estimate.
        </p>
