...
const EstimateReviewPanel = ({ tasks, lineItems }: Props) => {
  ...
  const totals = lineItems.reduce(
    (acc, item) => {
      const material =
        item.quantity && item.material_unit_price
          ...
          : 0;
      const labor =
        item.labor_hours && item.labor_rate ? item.labor_hours * item.labor_rate : 0;
    ...
    { material: 0, labor: 0, laborHours: 0, equipment: 0, disposal: 0, total: 0 },
  );

  const subtotal = totals.material + totals.labor + totals.equipment + totals.disposal;
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 space-y-6">
        ...
        <div className="rounded-lg border border-gray-100 p-4">
          <div className="text-sm font-semibold text-gray-800">Estimate totals</div>
          <div className="mt-2 grid gap-2 text-sm text-gray-600 md:grid-cols-3">
            ...
            <div>Equipment/Disposal: {formatCurrency(totals.equipment + totals.disposal)}</div>
