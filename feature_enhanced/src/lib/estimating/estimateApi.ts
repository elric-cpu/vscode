...
import type { EstimateLineItem, EstimateQuestion, EstimateTask, ParseResult } from "./types";
...
export const updateLineItem = async (
  ...
  await supabase.from("estimate_line_items").update(updates).eq("id", lineItemId);
};

export const fetchProductionRates = async () => {
  const { data, error } = await supabase.from("estimate_production_rates").select("*");
  if (error) throw error;
  return data || [];
};

export const fetchLaborRates = async () => {
  const { data, error } = await supabase.from("estimate_labor_rate_cards").select("*");
  if (error) throw error;
  return data || [];
};
