...
export const initDocumentUpload = async (payload) => {
  const { data, error } = await supabase.functions.invoke("estimate-upload-init", {
    body: payload,
  });
...
export const completeDocumentUpload = async (payload) => {
  const { data, error } = await supabase.functions.invoke("estimate-upload-complete", {
    body: payload,
  });
  if (error) throw error;
  return data;
...
export const fetchQuestions = async (payload) => {
  const { data, error } = await supabase.functions.invoke("estimate-questions", {
    body: payload,
  });
  if (error) throw error;
  return data;
...
export const submitAnswers = async (payload) => {
  const { data, error } = await supabase.functions.invoke("estimate-recompute", {
    body: payload,
  });
  if (error) throw error;
  return data;
