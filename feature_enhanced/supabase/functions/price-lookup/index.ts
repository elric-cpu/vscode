...
import { fetchScrapedPrice, getCachedPrice, getSeedPrice, upsertCache } from "../_shared/pricing.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { item_key, location_zip = null, unit = null, quantity = 1 } = payload;

    if (!item_key) {
      return new Response(
        JSON.stringify({ error: "Missing item_key." }),
