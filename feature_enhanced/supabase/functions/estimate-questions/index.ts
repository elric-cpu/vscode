...
serve(async (req) => {
  ...
  try {
    ...
    const { data: definitions } = await supabase
      ...
      .select("*")
      .or(`organization_id.is.null,organization_id.eq.${version.organization_id}`);
    ...
    const answerPayload = (answers || []).map((row) => row.answer).filter(Boolean);
    ...
    return jsonResponse({ questions: [...globalQuestions, ...questionInstances] });
  } catch (err) {
