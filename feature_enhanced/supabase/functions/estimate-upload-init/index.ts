...
serve(async (req) => {
  ...
  try {
    ...
    const { project_id: projectId, file_name: fileName, mime_type: mimeType, size_bytes: sizeBytes } = payload || {};
    ...
    const memberCheck = await requireOrgMember(supabase, project.organization_id, user.id);
    ...
    if (signedError || !signed) {
      return jsonResponse({ error: signedError?.message || "Unable to create upload URL" }, 500);
    }
