...
serve(async (req) => {
  ...
  try {
    ...
    const { estimate_id: estimateId, estimate_version_id: versionId } = payload || {};

    if (!estimateId && !versionId) {
      return jsonResponse({ error: "Missing estimate_id or estimate_version_id" }, 400);
    }
    ...
    const memberCheck = await requireOrgMember(supabase, estimate.organization_id, user.id);
    ...
    const excerptIds = new Set([
      ...
      ...(lineItems || []).map((item) => item.source_excerpt_id).filter(Boolean),
    ]);
