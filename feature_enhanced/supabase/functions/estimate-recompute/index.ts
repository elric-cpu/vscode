...
import { createAdminClient, getUserFromRequest, requireOrgMember, requireStaffRole } from "../_shared/utils.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const memberCheck = await requireOrgMember(supabase, version.organization_id, user.id);
    ...
    const definitionMap = new Map(definitions.map((def) => [def.id, def.field_key]));
    ...
    if (orchestratorUrl) {
      const response = await fetch(`${orchestratorUrl.replace(/\/$/, "")}/recompute`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(orchestratorSecret ? { "x-orchestrator-key": orchestratorSecret } : {}),
        },
        body: JSON.stringify({
          estimate_version_id: versionId,
      ...
      await supabase.from("audit_log").insert({
        ...
        payload: { estimate_version_id: versionId, new_version_id: data?.version_id },
      });
